...
import { createContext, Component, createElement, useRef, useContext, useState, useEffect, Fragment } from 'react';
...
function logIncorrectBinding(token, binding) {
    ...
    logError("Binding [".concat(tokenName, ", ").concat(bindingName, "] is incorrect."));
}
/**
 * @internal
 */
function logNotFoundDependency(token) {
    ...
    logError("Dependency ".concat(name, " is not found.\nPlease register ").concat(name, " in some Provider e.g.\n@provider([").concat(name, ", ").concat(name, "])\nclass App extends React.Component { /*...*/ }"));
}
/**
 * @internal
 */
function logNotFoundProvider(target) {
    if (isReactComponent(target)) {
        ...
        logError("Provider is not found.\n  Please define Provider and set ".concat(name_1, ".contextType = InjectorContext e.g.\n  @provider([MyService, MyService])\n  class App extends React.Component { /*...*/ }\n  class ").concat(name_1, " extends React.Component {\n    static contextType = InjectorContext;\n  }"));
    }
    else {
...
function logInvalidMetadata(target, token) {
    ...
    logError("".concat(tokenName, " is not a valid dependency.\nPlease specify ES6 class as property type e.g.\nclass MyService {}\nclass ").concat(targetName, " {\n  @inject myService: MyService;\n}"));
}
    ...
    if (isReactComponent(prototype)) {
        ...
        if (constructor.contextType !== InjectorContext) {
            if (constructor.contextType) {
                logError("Decorator tries to overwrite existing ".concat(className_1, ".contextType"));
            }
            else {
                Object.defineProperty(constructor, 'contextType', {
                    ...
                    set: function () {
                        logError("You are trying to overwrite ".concat(className_1, ".contextType = InjectorContext"));
                    }
                });
            }
        }
...
function toClass(constructor) {
    if ((process.env.NODE_ENV !== "production")) {
        if (!isFunction(constructor)) {
            logError("Class ".concat(getDebugName(constructor), " is not a valid dependency"));
        }
    }
    ...
    if ((process.env.NODE_ENV !== "production")) {
        if (factory) {
            if (!Array.isArray(depsOrFactory)) {
                logError("Dependency array ".concat(getDebugName(depsOrFactory), " is invalid"));
            }
            if (!isFunction(factory)) {
                logError("Factory ".concat(getDebugName(factory), " is not a valid dependency"));
            }
        }
        else if (!isFunction(depsOrFactory)) {
            logError("Factory ".concat(getDebugName(depsOrFactory), " is not a valid dependency"));
        }
    }
...
function toExisting(token) {
    if ((process.env.NODE_ENV !== "production")) {
        if (!isFunction(token)) {
            logError("Token ".concat(getDebugName(token), " is not a valid dependency injection token"));
        }
    }
...
function addBindings(bindingMap, definitions) {
    definitions.forEach(function (definition) {
        var _a;
        var token, binding;
        if (typeof definition == 'object' && definition.token && definition.binding) {
    ...
    return function (Wrapped) {
        ...
        var Provider = /** @class */ (function (_super) {
            ...
            Provider.prototype.render = function () {
                return createElement(InjectorContext.Provider, { value: this.state }, createElement(Wrapped, this.props));
            };
            /**
        ...
        if ((process.env.NODE_ENV !== "production")) {
            Provider.displayName = "Provider(".concat(Wrapped.displayName || Wrapped.name, ")");
            Object.defineProperty(Provider, 'contextType', {
                get: function () {
                    return InjectorContext;
                },
                set: function () {
                    logError("You are trying to overwrite ".concat(Provider.displayName, ".contextType = InjectorContext"));
                }
            });
        }
        else {
    ...
    registrationQueue.push(function () {
        if ((process.env.NODE_ENV !== "production")) {
            ...
            if (!isFunction(provider_1) || !(provider_1.prototype instanceof Injector)) {
                logError("".concat(getDebugName(provider_1), " is not a valid Provider. Please use:\n") + "@registerIn(() => MyProvider)\n" + "class ".concat(getDebugName(constructor), " {}\n"));
            }
            else {
...
function useInstance(token) {
    ...
    var injector = (_a = useContext(InjectorContext)) === null || _a === void 0 ? void 0 : _a.injector;
    if ((process.env.NODE_ENV !== "production")) {
        if (!injector) {
            logNotFoundProvider();
        }
    ...
    var _b = useState({}), updater = _b[1];
    var refUpd = useRef({ updater: updater });
    useEffect(function () {
            ...
            if (!injector._instanceMap.has(token)) {
                ...
                while (i) {
                    if (i._instanceMap.has(token)) {
                        publisher = i;
                        event = i._childNotifications.on(function () { return refUpd.current.updater({}); });
                    }
                    i = i._parent;
                }
            }
        ...
        return function () {
            refUpd.current.updater = function () { };
            if (publisher) {
...
function useInstances() {
    ...
    var result = ref.current || (ref.current = tokens.map(function (token) { return getInstance(injector, token); }));
    var _b = useState({}), updater = _b[1];
    var refUpd = useRef({ updater: updater });
    useEffect(function () {
        // if token found in nearest provider - no update is required - useContext already invoke rerender,
        ...
        return function () {
            refUpd.current.updater = function () { };
            subscriptions.forEach(function (s) { return s.publisher._childNotifications.off(s.event); });
        };
    }, []);
    return result;
}
...
var ComponentWithServices = function (_a) {
    var services = _a.services, children = _a.children;
    var ref = useRef(provider.apply(void 0, services)(ComponentWithService));
    return createElement(ref.current, null, children);
};

export { ComponentWithServices, inject as Inject, InjectedService, LiteEventImpl, provider as Provider, registerIn as RegisterIn, inject, provider, registerIn, toClass, toExisting, toFactory, toValue, useInstance, useInstances };
